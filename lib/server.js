 ...
 */
Server.prototype.build = function (entry) {
...
Server.prototype.handleRender = function (req, res, next) {
  var base = path.join(req.app.mountpath, 'build/');

  this.render(base, function (err, html) {
...
Server.prototype.handleBuild = function (req, res, next) {
  ...
  this.build(entry).sourceMap('inline').run(function (err, src) {
    if (err) return next(err);
